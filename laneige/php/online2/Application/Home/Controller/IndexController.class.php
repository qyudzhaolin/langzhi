...
class IndexController extends Controller
{
    ...
    public function saveInfo()
    {
        ...
        if($insert_id){
            // $this->sendsms($phone);
            $id = mysql_insert_id();
            $counter = M('city')->where("id=".$id)->getField('counter');
            $activetime = M('city')->where("id=".$id)->getField('activetime');
    ...
    $signPackage = getSign($url, '15144605209907', 'h0aeMdAMN9aNmezJ', $this->appid);
