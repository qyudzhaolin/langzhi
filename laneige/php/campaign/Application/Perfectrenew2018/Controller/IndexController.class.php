...
class IndexController extends Controller {
    ...
    protected $card_id = "pkGCUjs8gLFWkw72kLSJgrdYrUjE";
    
    public function index(){
        if(!session('openid')){
            $url = "http://campaign.laneige.com/Perfectrenew2018/index/getcode";
            if($_GET['smtid']){
            ...
            $url = "http://api.weq.me/wx/authorize.php?id=15131578769769&scope=snsapi_base&redirect_uri=".urlencode($url);
            redirect($url);
        }else{
            if(I('get.openid')){
    ...
    public function getcode(){
        if(I('get.openid')){
            session('openid', I('get.openid'));
            $url = 'http://campaign.laneige.com/Perfectrenew2018/';
            if($_GET['smtid']){
                $url .= "?smtid=".$_GET['smtid'];
            }else{
                if($_GET['smt_cp']) $url .= "?smt_cp=".$_GET['smt_cp'];
    ...
    protected function getShareInfo(){
        ...
        if($is_weixin) {
            ...
            $signPackage = getSign($url, '15131578769769', 'Nc0sQ19GhWbsK94O', $this->appid);
    ...
    public function get_access_token(){
        $token = get_token('15131578769769', 'Nc0sQ19GhWbsK94O');
